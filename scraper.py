...
def main():  # pragma: no cover
  ...
  lockfile = os.path.join(
      args.lockfile_dir[0],
      '{}_{}.lock'.format(args.rsync_host[0], args.rsync_module[0]))
  acquire_lock_or_die(lockfile)
  atexit.register(os.remove, lockfile)

  # If the destination directory does not exist, make it exist.
  destination = os.path.join(args.data_dir[0], args.rsync_host[0],
                             args.rsync_module[0])
  if not os.path.isdir(destination):
    os.makedirs(destination)

  # Get the file list and then the files from the server.
  rsync_url = 'rsync://{}:{}/{}/'.format(args.rsync_host[0], args.rsync_port,
                                         args.rsync_module[0])
  files = list_rsync_files(args.rsync_binary[0], rsync_url, destination)
  progress_level = get_progress_from_spreadsheet(args.spreadsheet[0],
                                                 args.rsync_host[0],
                                                 args.rsync_module[0])
  files = remove_older_files(progress_level, files)
  download_files(args.rsync_binary[0], rsync_url, files, destination)
