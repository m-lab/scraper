...
class TestScraper(unittest.TestCase):
  ...
  def test_args(self):
    ...
    args = scraper.parse_cmdline(['--rsync_host', RSYNC_HOST,
                                  '--lockfile_dir', LOCKFILE_DIR,
                                  '--rsync_module', RSYNC_MODULE,
                                  '--data_dir', DATA_DIR,
                                  '--spreadsheet', SPREADSHEET])
    self.assertEqual(args.rsync_host[0], RSYNC_HOST)
    self.assertEqual(args.lockfile_dir[0], LOCKFILE_DIR)
    self.assertEqual(args.rsync_module[0], RSYNC_MODULE)
    self.assertEqual(args.data_dir[0], DATA_DIR)
    self.assertEqual(args.spreadsheet[0], SPREADSHEET)
...
-rw-r--r--            103 2016/01/06 22:32:01 2016/01/06/20160106T22:31:57.229531000Z_:0.cputime.gz'''
    ...
    files = scraper.list_rsync_files('/usr/bin/rsync', 'localhost', '/tmp')
    self.assertEqual(['.',
                      '2016',
                      '2016/01',
                      '2016/01/06',
  ...
  @mock.patch('subprocess.check_output')
  def test_list_rsync_files_fails(self, patched_subprocess):
    patched_subprocess.side_effect = subprocess.CalledProcessError(
        1, ['/usr/bin/rsync', 'arg'], 'Mock error')
    with self.assertRaises(SystemExit):
      files = scraper.list_rsync_files('/usr/bin/rsync', 'localhost', '/tmp')
  ...
  def test_remove_older_files(self):
    ...
    filtered = scraper.remove_older_files(
        datetime.datetime(2016,10,25).date(),
        files)
    self.assertEqual(
        filtered,
        [ '2016/10/26/20161026T17:52:59.797186000Z_eb.measurementlab.net:35192.s2c_snaplog.gz',
  ...
  def test_download_files_fails_and_dies(self):
    with self.assertRaises(SystemExit):
      files = scraper.download_files('/bin/false', 'localhost/', ['2016/10/26/DNE1', '2016/10/26/DNE2'], '/tmp')


  def test_download_files_with_empty_does_nothing(self):
    # /bin/false would cause a SystemExit if run
    files = scraper.download_files('/bin/false', 'localhost/', [], '/tmp')
  ...
  def test_download_files(self, patched_check_call):
    ...
    def verify_contents(args):
      self.assertEqual(files, [x.strip() for x in file(args[2]).readlines()])
    patched_check_call.side_effect = verify_contents
    files = scraper.download_files('/bin/true', 'localhost/', ['2016/10/26/DNE1', '2016/10/26/DNE2'], '/tmp')
  ...
  def test_find_all_days_to_upload(self):
    try:
      ...
      to_upload = list(sorted(scraper.find_all_days_to_upload(temp_d, date)))
      self.assertEqual(to_upload, [datetime.date(2015, 10, 31),
